...
else
{
	...
	$oInformationsystem_Item->queryBuilder()
 		...
 		->where('active', '=', 1)
 		->where('id', '!=', $Informationsystem_Controller_Show->item);
	$aInformationsystem_Items = $oInformationsystem_Item->findAll();

	if (count ($aInformationsystem_Items))
	{
		$xmlconcentrator = Core::factory('Core_Xml_Entity')->name('concentrator');
		$Informationsystem_Controller_Show->addEntity($xmlconcentrator);
		foreach ($aInformationsystem_Items as $oInformationsystemItem)
		{
		$oInformationsystemItem->addForbiddenTags(array('description' , 'text'));
		}
	}
}
