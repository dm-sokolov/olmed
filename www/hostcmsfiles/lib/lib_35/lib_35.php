...
if ($Shop_Controller_Show->item != 0)
{
	...
	if (Core_Array::getPost('update'))
	{
		...
		$oShop_Item->name = Core_Array::getPost('name');
		$oShop_Item->text = Core_Array::getPost('text');
							...
							foreach ($aFileData['name'] as $key => $sFileName)
							{
								...
								if (Core_File::isValidExtension($sFileName, array('JPG', 'JPEG', 'GIF', 'PNG')))
								{
									...
									try
									{
										...
										Core_Image::resizeImage($aFileData['tmp_name'][$key], $oShop->image_large_max_width, $oShop->image_large_max_height, $oShop_Item->getItemPath() . $oShop_Item_Property_List->getLargeFileName($oShop_Item, $oFileValue, $sFileName));
										$oFileValue->file = $oShop_Item_Property_List->getLargeFileName($oShop_Item, $oFileValue, $sFileName);

										Core_Image::resizeImage($aFileData['tmp_name'][$key], $oShop->image_small_max_width, $oShop->image_small_max_height, $oShop_Item->getItemPath() . $oShop_Item_Property_List->getSmallFileName($oShop_Item, $oFileValue, $sFileName));
