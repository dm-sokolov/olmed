...
if (Core_Array::getPost('add_ticket'))
{
	...
	$oHelpdesk_Ticket->notify_change_status = intval(Core_Array::getPost('notify_change_status', 0));
	$oHelpdesk_Ticket->send_email = intval(Core_Array::getPost('send_email', 0));
	...
	$oHelpdesk_Message->subject = strval(Core_Array::getPost('subject'));
...
if ($Helpdesk_Controller_Show->ticket && Core_Array::getPost('send_message'))
{
	...
	if ($oHelpdesk_Ticket->siteuser_id == $oSiteuser->id)
	{
		...
		$oHelpdesk_Message->subject = strval(Core_Array::getPost('message_subject'));
