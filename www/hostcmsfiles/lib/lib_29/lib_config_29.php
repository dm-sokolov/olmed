...
class My_Core_Lastmode_Sitemap extends Core_Sitemap
{
...

	{
...

		{
			...
			case 6 : $sChangefreq = 'never'; break;
               ...
               if( substr_count($loc, "price") == 0 && !(substr_count($loc, "otzivy") && substr_count($loc, "/") > 5)  && !(preg_match('/[A-Z]/', $loc)) ) {
		$content = "<url>\n";
		...
		$content .= Core_Event::getLastReturn();
               ...
               $cur_date = date("M d Y");
               $timestamp = rand( strtotime("Aug 22 2019"), strtotime($cur_date) ); 
               $random_Date = date("Y-m-d", $timestamp ); 

		$content .= "<loc>{$loc}</loc>\n" . 
                "<lastmod>" . $random_Date . "</lastmod>".
...
try
{
	$oCore_Sitemap = new My_Core_Lastmode_Sitemap($oSite);
