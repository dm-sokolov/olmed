...
if ($Shop_Controller_Show->item == 0)
{
	...
	if ($Shop_Controller_Show->group && Core_Array::getPost('send_ad'))
	{
		...
		$oShop_Item->name = strval(Core_Array::getPost('name'));
		$oShop_Item->text = strval(Core_Array::getPost('text'));
			...
			if (is_array($aFileData) && isset($aFileData['name']))
			{
				if (Core_File::isValidExtension($aFileData['name'], array('JPG', 'JPEG', 'GIF', 'PNG')))
				{
					try
					{
						...
						Core_Image::resizeImage($aFileData['tmp_name'], $oShop->image_large_max_width, $oShop->image_large_max_height, $oShop_Item->getItemPath() . $sLargeImageFile);
						$oShop_Item->image_large = $sLargeImageFile;
						$oShop_Item->setLargeImageSizes();

						Core_Image::resizeImage($aFileData['tmp_name'], $oShop->image_small_max_width, $oShop->image_small_max_height, $oShop_Item->getItemPath() . 'small_'.$sLargeImageFile);
			...
			foreach ($aProperties as $oProperty)
			{
				...
				if ($oProperty->type != 10)
				{
					...
					switch ($oProperty->type)
					{
						...
						case 4: // Textarea
						case 6: // Wysiwyg
							$oProperty_Value->value(strval(Core_Array::getPost("property_{$oProperty->id}")));
							$oProperty_Value->save();
						break;
						case 8: // Date
								...
								foreach ($aFileData['name'] as $key => $sFileName)
								{
									...
									if (Core_File::isValidExtension($sFileName, array('JPG', 'JPEG', 'GIF', 'PNG')))
									{
										...
										try
										{
											...
											Core_Image::resizeImage($aFileData['tmp_name'][$key], $oShop->image_large_max_width, $oShop->image_large_max_height, $oShop_Item_Property_List->getDirPath($oShop_Item) . $oShop_Item_Property_List->getLargeFileName($oShop_Item, $oFileValue, $sFileName));

											$oFileValue->file = $oShop_Item_Property_List->getLargeFileName($oShop_Item, $oFileValue, $sFileName);

											Core_Image::resizeImage($aFileData['tmp_name'][$key], $oShop->image_small_max_width, $oShop->image_small_max_height, $oShop_Item_Property_List->getDirPath($oShop_Item) . $oShop_Item_Property_List->getSmallFileName($oShop_Item, $oFileValue, $sFileName));
