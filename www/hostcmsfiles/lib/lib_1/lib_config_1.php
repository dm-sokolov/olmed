...
$aKeywords[] =  $oInformationsystem->name;
}
...
{
	array_push($aTitle, $pageName . ' ' . ($Informationsystem_Controller_Show->page + 1));
}
if ( count($aTitle) )
{
...
if($oInformationsystem_Group->id == 0) {
$elemcnt = $oInformationsystem->informationsystem_items->getCountByActive(1);
}
else {
$elemcnt = $oInformationsystem_Group->informationsystem_items->getCountByActive(1);
}

if ( $Informationsystem_Controller_Show->limit <  $elemcnt) {

if($Informationsystem_Controller_Show->group==0 || $Informationsystem_Controller_Show->group && !$Informationsystem_Controller_Show->item) {

    $links=$Informationsystem_Controller_Show->page +1; // Номер текущей страницы в пагинации
    $linki = ''; //Теги, которые пойдут в шаблон
    //Всего страниц
if($oInformationsystem_Group->id == 0) {
    $pagecnt = ceil(($oInformationsystem->informationsystem_items->getCountByActive(1)) / ($Informationsystem_Controller_Show->limit));  
   }
else {
  $pagecnt = ceil(($oInformationsystem_Group->informationsystem_items->getCountByActive(1)) / ($Informationsystem_Controller_Show->limit)); 
}   

$pattern = $oInformationsystem->Structure->getPath();
if($oInformationsystem_Group->id) {
   $pattern .= $oInformationsystem_Group->getPath(); //URL для группы информационной системы 
}
