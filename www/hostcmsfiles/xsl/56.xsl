...
<xsl:stylesheet version="1.0"
			...
			<xsl:if test="price != 0">
				...
				<xsl:value-of select="format-number(price, '### ##0,00', 'my')"/><xsl:text> </xsl:text><xsl:value-of select="currency"/><xsl:text> </xsl:text>
					...
					<xsl:if test="price_tax != price">
						<span class="oldPrice">
							<xsl:value-of select="format-number(price_tax, '### ##0,00', 'my')"/><xsl:text> </xsl:text><xsl:value-of select="currency" />
	...
	<div class="shop_property">Вес товара: <span hostcms:id="{@id}" hostcms:field="weight" hostcms:entity="shop_item"><xsl:value-of select="weight"/></span><xsl:text> </xsl:text><span><xsl:value-of select="/shop/shop_measure/name"/></span></div>
		...
		<div class="shop_property">В наличии: <span><xsl:value-of select="rest - reserved"/><xsl:text> </xsl:text><xsl:value-of select="shop_measure/name"/></span></div>
			</xsl:if>
...
<div class="shop_discount"><xsl:value-of select="name"/><xsl:text> </xsl:text><span><xsl:value-of select="percent"/>%</span></div>
	</xsl:template>
