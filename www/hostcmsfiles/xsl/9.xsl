...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/informationsystem">
		...
		<xsl:choose>
			<xsl:when test="$group = 0">
				...
				<span>(орфография и пунктуация авторов вопросов сохранены)</span>
				<div class="hr"></div>
		...
		<p class="button-one">
			<a href="#form_3" class="modal-button" style="color:#fff;text-decoration:none;">Добавить вопрос</a>
		</p>
			...
			<div class="comment">
				...
				<form action="./" method="post">
					...
					<div class="row">
						<div class="caption"></div>
						<div class="field">
							<input type="submit" name="submit_question" value="Добавить вопрос" class="button"/>
						</div>
					</div>
	...
	<xsl:template match="informationsystem_item">
		<xsl:variable name="blue"><xsl:choose>
				<xsl:when test="position() mod 2 = 0">blue</xsl:when>
				<xsl:otherwise></xsl:otherwise>
		</xsl:choose></xsl:variable>
		<div class="qa-question {$blue}">
			<xsl:value-of disable-output-escaping="yes" select="text"/>
			<div class="ugolok"></div>
			<div class="name-date" align="center">
				<xsl:choose>
					<xsl:when test="property_value[tag_name='girl']/value = 1"><img src="/img/qa-girl.png"/></xsl:when>
					<xsl:otherwise><img src="/img/qa-man.png"/></xsl:otherwise>
				</xsl:choose><br />
