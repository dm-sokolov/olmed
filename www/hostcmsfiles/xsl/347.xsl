...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="form">
		...
		<script type="text/javascript">
			$(function() {
			var sPage3 = location.href.replace(/https?:\/\//i, "");
			$('input[name="sPage3"]').val(sPage3);
			var siteID = <xsl:value-of select="site_id" />;
			...
			$.sendForm('/callback/',$('#formBox<xsl:value-of select="@id" />'), <xsl:value-of select="@id" />, 347, d);
			if (siteID == 1) {
		...
		<div id="formBox{@id}" class="blue-gradient sign-up-box">
			<div style="border: 1px solid rgba(0, 0, 0, 0.2);padding:30px 0;">
					...
					<xsl:choose>
						...
						<xsl:otherwise>
							...
							<div style="width:70%;margin:0 auto;">
								...
								<button style="width:200px;height:50px;" name="{button_name}" value="{button_value}" type="submit" class="form-control btn btn-primary d-block mx-auto">
	...
	<xsl:template match="form_field">
		...
		<xsl:if test="type != 7 and type != 8">
			...
			<div class="form-group mt-3">
				...
				<!-- Список -->
				<xsl:if test="type = 6">
					<select name="{name}" style="height: 45px;border-radius: 2px;">
						<xsl:if test="obligatory = 1">
							<xsl:attribute name="class">form-control required</xsl:attribute>
							<xsl:attribute name="title">Заполните поле <xsl:value-of select="$name" /></xsl:attribute>
						</xsl:if>
