...
<xsl:stylesheet version="1.0"
	...
	<xsl:template name="for">
		...
		<xsl:if test="$items_count &gt; $limit and ($page + $post_count_page + 1) &gt; $i">
			...
			<xsl:if test="$page != 0 and $i = $page"><xsl:variable name="prev_number_link"><xsl:if test="($page) != 0">page-<xsl:value-of select="$i"/>/</xsl:if></xsl:variable><a href="{/shop/structure/link}{$prev_number_link}" id="id_prev"></a></xsl:if>
