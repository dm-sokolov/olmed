					...
					<xsl:otherwise>
						...
						<div class="comment">
							...
							<form name="form{@id}" id="form{@id}" class="validate" action="/callback/?ajaxForm={@id}" method="post">
								...
								<!-- Обратный звонок / письмо директору #callback #letter -->
								<div class="row">
									<div class="caption"></div>
									<div class="field">
										<input name="{button_name}" value="{button_value}" type="submit" class="button" onclick="yaCounter20420221.reachGoal('callback'); return true;" />
									</div>
								</div>
