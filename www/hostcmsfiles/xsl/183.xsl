...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="helpdesk">
		...
		<form method="get" action="{/helpdesk/url}">
			...
			<p>
				<input type="submit" class="button" value="Показать" name="apply_filter"/>
			</p>
		</form>
		...
		<xsl:choose>
			<xsl:when test="helpdesk_ticket/node()">
				<table border="0" cellpadding="0" cellspacing="0" class="table_border_hostcms">
					<tr>
		...
		<div class="comment">
			...
			<form action="{/document/url}" name="" method="post" enctype="multipart/form-data">
				...
				<div class="row">
					...
					<div class="field">
						<p>
							<input id="check_1" type="checkbox" name="notify_change_status" checked=""/>
							<label for="check_1">Посылать сообщения о смене статуса на e-mail</label>
						</p>
						<p>
							<input id="check_2" type="checkbox" name="send_email" checked=""/>
