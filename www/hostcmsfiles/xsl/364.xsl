...
<xsl:stylesheet version="1.0"
	...
	<!-- СписокУслугИнфосистемыNEW -->
	<xsl:variable name="group" select="/informationsystem/group" />
	<xsl:variable name="current_group" select="/informationsystem//informationsystem_group[@id=$group]" />
	
	<xsl:variable name="count" select="count(/informationsystem//*[@id][parent_id = $group or informationsystem_group_id = $group][not(property_value[tag_name='hide-in-list'] and property_value[tag_name='hide-in-list']/value = 1)])"/>
