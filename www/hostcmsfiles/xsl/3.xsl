		...
		<div class="item-news">
			<xsl:choose>
				<xsl:when test="image_small!=''">
					<a href="{url}" class="news_pict" title="{name}">
						<img src="{dir}{image_small}" class="news_img" alt="{name}" align="left"/>
					</a>
				</xsl:when>
				<xsl:otherwise>
					<a href="{url}" class="news_pict" title="{name}">
						<img src="/img/noimage.png" class="news_img" alt="{name}" align="left" width="202"/>
					</a>
				</xsl:otherwise>
			</xsl:choose>
			...
			<a href="{url}" class="more" title="{name}">Читать далее...</a>
		...
		<xsl:if test="$items_count &gt; $limit and ($page + $post_count_page + 1) &gt; $i">
			...
			<xsl:if test="$page != 0 and $i = $page">
				<xsl:variable name="prev_number_link">
					<xsl:choose>
						<!-- Если не нулевой уровень -->
						<xsl:when test="not(($page) = 0) and not (($page) = 1)">page-<xsl:value-of select="$i"/>/</xsl:when>
						<!-- Иначе если нулевой уровень - просто ссылка на страницу со списком элементов -->
						<xsl:otherwise></xsl:otherwise>
					</xsl:choose>
				</xsl:variable>
