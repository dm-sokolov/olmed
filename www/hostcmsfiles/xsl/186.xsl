...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="siteuser">
		...
		<!-- <xsl:apply-templates select="affiliate_items_data" /> -->
	</xsl:template>
	...
	<xsl:template match="affiliats/siteuser">
		...
		<li>
			...
			<xsl:value-of select="format-number(sum(transactions/shop_siteuser_transaction[shop_id = $shop_id]/amount_base_currency), '0.##')" /><xsl:text> </xsl:text>
