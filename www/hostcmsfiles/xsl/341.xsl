		...
		<div id="formBox{@id}">
			<div class="blue-gradient sign-up-box p-4">
				...
				<script type="text/javascript">
					$(function() {
					var sPage2 = location.href.replace(/https?:\/\//i, "");
					$('input[name="sPage2"]').val(sPage2);
					var siteID = <xsl:value-of select="site_id" />;
					console.log('<xsl:value-of select="@id" />');
					$('#form<xsl:value-of select="@id" />').validate({
					focusInvalid: true,
					errorClass: "input_error",
					submitHandler: function(form) {
					var d = $(form).serialize();
					$.sendForm('/callback/',$('#formBox<xsl:value-of select="@id" />'), <xsl:value-of select="@id" />, 313, d);
					if (siteID == 1) {
	...
	<xsl:template match="form_field">
		...
		<xsl:if test="type != 7 and type != 8">
			...
			<div class="col-sm-6">
				...
				<!-- Список -->
				<xsl:if test="type = 6">
					<select name="{name}" class="city-list">
						<xsl:if test="obligatory = 1">
