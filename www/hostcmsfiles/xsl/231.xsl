	...
	<xsl:template match="/informationsystem/informationsystem_item">
		...
		<div class="item-news">
					...
					<xsl:when test="image_large!=''">
						<div id="gallery">
							<a href="{dir}{image_large}" target="_blank">
								<img src="{dir}{image_small}" align="left" class="news_img"/>
							</a>
						</div>
					</xsl:when>
					<xsl:otherwise>
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					<div class="inner-text" hostcms:id="{@id}" hostcms:field="text" hostcms:entity="informationsystem_item" hostcms:type="wysiwyg">
						<xsl:value-of disable-output-escaping="yes" select="text"/>
					</div>
				</xsl:otherwise>
			</xsl:choose>
			<a href="#form_3" class="modal-button button">Задать вопрос</a>
