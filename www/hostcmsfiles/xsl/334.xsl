...
<xsl:stylesheet version="1.0"
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:if test="other_name/node()">
					<h2><xsl:value-of disable-output-escaping="yes" select="other_name" /><xsl:value-of select="$city" /></h2>
				</xsl:if>
	...
	<xsl:template match="informationsystem_group">
		<xsl:if test="not(informationsystem_group)">
			<div class="h4">
...
<div class="specialist-item col-sm-6 mb-4">
	<div class="row">
		...
		<div class="specialist-info col-lg-7">
			...
			<button class="btn btn-primary" onclick="$.showXslTemplate('/callback/', 84, 357, ' ', '{$doctor}'); return false;">Запись к врачу</button>
		</div>
	</div>
</div>
