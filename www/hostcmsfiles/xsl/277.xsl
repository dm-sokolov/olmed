...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/informationsystem">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<h1 hostcms:id="{$group}" hostcms:field="name" hostcms:entity="informationsystem_group" style="color:#046391;font-size:30px;text-decoration:underline;line-height:35px;">
					<xsl:value-of select=".//informationsystem_group[@id=$group]/name"/>
				</h1>
				<xsl:if test=".//informationsystem_group[@id=$group]/image_small !=''">
					<div class="u-pict">
						<xsl:if test="informationsystem_group[@id=52 or @id=85]">
							<xsl:attribute name="style">margin-bottom:20px;</xsl:attribute>
						</xsl:if>
						<img src="{.//informationsystem_group[@id=$group]/dir}{.//informationsystem_group[@id=$group]/image_small}" align="left" /><div class="u-bg"></div>
						<xsl:if test="informationsystem_group/@id!=102">
							<div class="vrach"><xsl:value-of disable-output-escaping="yes" select=".//informationsystem_group[@id=$group]/property_value[tag_name ='vrach']/value"/><br /><span><xsl:value-of disable-output-escaping="yes" select=".//informationsystem_group[@id=$group]/property_value[tag_name ='vrach']/description"/></span>
								
								<!-- <br /><a href="#form_3" class="modal-button button">Задать вопрос</a>-->
				...
				<xsl:if test="page = 0 and .//informationsystem_group[@id=$group]/description != ''">
					<div hostcms:id="{$group}" hostcms:field="description" hostcms:entity="informationsystem_group" hostcms:type="wysiwyg" class="inner-text"><xsl:value-of disable-output-escaping="yes" select=".//informationsystem_group[@id=$group]/description"/></div>
				</xsl:if>
			</xsl:otherwise>
		</xsl:choose>
		...
		<ul>
			<xsl:for-each select=". | following-sibling::informationsystem_group[position() &lt; $n]">
				<li>
					...
					<div class="desc-little"><xsl:value-of disable-output-escaping="yes" select="property_value[tag_name ='desc_little']/value"/></div>
				</li>
			</xsl:for-each>
		</ul>
	</xsl:template>
