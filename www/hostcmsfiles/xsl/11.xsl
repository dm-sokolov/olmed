...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/informationsystem">
		...
		<script type="text/javascript">
			...
			$("#currentImage").attr('src',loading.src);
			$("#currentImage").attr('src',l.src);
			}
			
			setCurrentImage('<xsl:value-of disable-output-escaping="yes" select="informationsystem_item[1]/dir"/><xsl:value-of disable-output-escaping="yes" select="informationsystem_item[1]/image_small"/>');
		...
		<xsl:if test="count(informationsystem_item) > 0">
			...
			<!-- jQuery lightbox -->
			<div class="gal-spisok">
				<div class="large" align="center" id="setCurrent">
	...
	<xsl:template match="informationsystem_group" mode="groups">
		<li>
			<xsl:choose>
				...
				<a href="{url}" title="{name}"><img src="{dir}{image_small}" alt="{name}" /><div><xsl:value-of select="name"/></div></a>
				</xsl:when>
				<xsl:otherwise>
				<a href="{url}" title="{name}"><img src="/images/no-image.png" alt="{name}" /><div><xsl:value-of select="name"/></div></a>
				</xsl:otherwise>
			</xsl:choose>
		</li>
	</xsl:template>
	...
	<xsl:template match="informationsystem_item" mode="first">
		<a href="{dir}{image_large}" target="_blank"><img src="{dir}{image_large}" alt="{name}" title="{name}" id="currentImage" name="currentImage" /></a>
	</xsl:template>
	
	<xsl:template match="informationsystem_item">
		<li>
			<img src="{dir}{image_small}" alt="{name}" title="{name}" onclick="setCurrentImage('{dir}{image_large}')" />
		</li>
	</xsl:template>
