...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/informationsystem/informationsystem_item">
		...
		<xsl:if test="image_large!=''">
			<p>
				<img class="image" src="{dir}{image_large}" style="margin: 0 10px 10px 0px"/>
			</p>
		</xsl:if>
	...
	<xsl:template name="for_star">
		...
		<input type="radio" name="grade" value="{$i}" id="id_grade_{$i}"/><xsl:text> </xsl:text>
		<label for="id_igrade_{$i}">
