...
class Shop_Payment_System_Handler14 extends Shop_Payment_System_Handler
{
	...
	function ProcessResult()
	{
		...
		// Сравниваем хэши
		if (self::check(Core_Array::getPost('pg_sig', ''), "", $_POST, $this->_merchant_secret_key)
			&& Core_Array::getPost('pg_result', 0) == 1
			&& Shop_Controller::instance()->round(Core_Array::getPost('pg_net_amount', 0)) == $order_sum)
		{
			$this->_shopOrder->system_information = sprintf("Товар оплачен через Platron.\n\nИнформация:\n\nИдентификатор платежа в системе Platron: %s\n", Core_Array::getPost('pg_payment_id'));
