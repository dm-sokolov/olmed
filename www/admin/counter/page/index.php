...
// Код формы
$iAdmin_Form_Id = 99;
$sAdminFormAction = '/admin/counter/page/index.php';

$sCounterPath = '/admin/counter/index.php';

$oAdmin_Form = Core_Entity::factory('Admin_Form', $iAdmin_Form_Id);

$sTitle = Core::_('Counter.page');
...
(!isset($oAdmin_Form_Controller->request['admin_form_filter_from_416'])
	|| $oAdmin_Form_Controller->request['admin_form_filter_from_416'] == '')
		&& $oAdmin_Form_Controller->request['admin_form_filter_from_416'] = Core_Date::timestamp2date(time());

(!isset($oAdmin_Form_Controller->request['admin_form_filter_to_416'])
	|| $oAdmin_Form_Controller->request['admin_form_filter_to_416'] == '')
		&& $oAdmin_Form_Controller->request['admin_form_filter_to_416'] = Core_Date::timestamp2date(time());

$bSeveralDates = $oAdmin_Form_Controller->request['admin_form_filter_from_416'] != $oAdmin_Form_Controller->request['admin_form_filter_to_416'];
		...
		$.getMultiContent(aScripts, '/modules/skin/bootstrap/js/charts/flot/').done(function() {
			...
			foreach ($aObjects as $key => $oObject)
			{
				...
				popularDiagramData.push(
					{
						label: '<?php echo Core_Str::escapeJavascriptVariable(htmlspecialchars(strip_tags($oObject->pageBackend())))?>',
			...
			var placeholder = $("#<?php echo $sWindowId?> #popularDiagram");
