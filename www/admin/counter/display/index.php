...
// Код формы
$iAdmin_Form_Id = 114;
$sAdminFormAction = '/admin/counter/display/index.php';

$sCounterPath = '/admin/counter/index.php';

$oAdmin_Form = Core_Entity::factory('Admin_Form', $iAdmin_Form_Id);

$sTitle = Core::_('Counter.displays');
...
$oAdmin_Form_Dataset = new Admin_Form_Dataset_Entity(
	Core_Entity::factory('Counter_Display')
);

(!isset($oAdmin_Form_Controller->request['admin_form_filter_from_512'])
	|| $oAdmin_Form_Controller->request['admin_form_filter_from_512'] == '')
		&& $oAdmin_Form_Controller->request['admin_form_filter_from_512'] = Core_Date::timestamp2date(time());

(!isset($oAdmin_Form_Controller->request['admin_form_filter_to_512'])
	|| $oAdmin_Form_Controller->request['admin_form_filter_to_512'] == '')
		&& $oAdmin_Form_Controller->request['admin_form_filter_to_512'] = Core_Date::timestamp2date(time());

$bSeveralDates = $oAdmin_Form_Controller->request['admin_form_filter_from_512'] != $oAdmin_Form_Controller->request['admin_form_filter_to_512'];
	...
	<div class="col-xs-12 col-lg-8">
		<div class="widget counter">
			<div class="widget-body">
				<div class="row">
					<div class="col-xs-12">
						<div id="displaysDiagram" class="chart"></div>
					</div>
				</div>
			</div>
		</div>
		...
		$(function(){
			...
			$.getMultiContent(aScripts, '/modules/skin/bootstrap/js/charts/flot/').done(function() {
				...
				foreach ($aObjects as $key => $oObject)
				{
					...
					diagramData.push(
						{
							label: '<?php echo Core_Str::escapeJavascriptVariable($oObject->display)?>',
				...
				?>
				var placeholderDisplaysDiagram = $("#<?php echo $sWindowId?> #displaysDiagram");

				$.plot(placeholderDisplaysDiagram, diagramData, {
					series: {
						pie: {
							show: true,
							radius: 1,
				...
				placeholderDisplaysDiagram.bind("plothover", function (event, pos, obj) {
					if (!obj) {
						return;
					}
					$("#<?php echo $sWindowId?> #displaysDiagram span[id ^= 'pieLabel']").hide();
					$("#<?php echo $sWindowId?> #displaysDiagram span[id ^= 'pieLabel" + obj.seriesIndex + "']").show();
				});

				placeholderDisplaysDiagram.resize(function(){$("#<?php echo $sWindowId?> #displaysDiagram span[id ^= 'pieLabel']").hide();});
