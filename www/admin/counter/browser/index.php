...
// Код формы
$iAdmin_Form_Id = 112;
$sAdminFormAction = '/admin/counter/browser/index.php';

$sCounterPath = '/admin/counter/index.php';

$oAdmin_Form = Core_Entity::factory('Admin_Form', $iAdmin_Form_Id);

$sTitle = Core::_('Counter.browsers');
...
$oAdmin_Form_Dataset = new Admin_Form_Dataset_Entity(
	Core_Entity::factory('Counter_Browser')
);

$aSetting = Core_Array::get(Core::$config->get('counter_setting'), 'setting', array());

(!isset($oAdmin_Form_Controller->request['admin_form_filter_from_456'])
	|| $oAdmin_Form_Controller->request['admin_form_filter_from_456'] == '')
		&& $oAdmin_Form_Controller->request['admin_form_filter_from_456'] = Core_Date::timestamp2date(time());

(!isset($oAdmin_Form_Controller->request['admin_form_filter_to_456'])
	|| $oAdmin_Form_Controller->request['admin_form_filter_to_456'] == '')
		&& $oAdmin_Form_Controller->request['admin_form_filter_to_456'] = Core_Date::timestamp2date(time());

// Change filter date to single mode
// $oAdmin_Form_Controller->addFilter('date', array($oAdmin_Form_Controller, '_filterCallbackDateSingle'));

$bSeveralDates = $oAdmin_Form_Controller->request['admin_form_filter_from_456'] != $oAdmin_Form_Controller->request['admin_form_filter_to_456'];
	...
	?><div class="col-xs-12 col-lg-8">
		<div class="widget counter">
			<div class="widget-body">
				<div class="row">
					<div class="col-xs-12">
						<div id="browsersDiagram" class="chart"></div>
					</div>
				</div>
			</div>
		</div>
		...
		$(function(){
			...
			$.getMultiContent(aScripts, '/modules/skin/bootstrap/js/charts/flot/').done(function() {
				...
				foreach ($aObjects as $key => $oObject)
				{
					...
					diagramData.push(
						{
							label:'<?php echo Core_Str::escapeJavascriptVariable(htmlspecialchars(Core_Str::cut($oObject->browser, $aSetting['nameLen'])))?>',
				...
				?>
				var placeholderBrowsersDiagram = $("#<?php echo $sWindowId?> #browsersDiagram");

				$.plot(placeholderBrowsersDiagram, diagramData, {
					series: {
						pie: {
							show: true,
							radius: 1,
				...
				placeholderBrowsersDiagram.bind("plothover", function (event, pos, obj) {
					if (!obj) {
						return;
					}

					$("#<?php echo $sWindowId?> #browsersDiagram span[id ^= 'pieLabel']").hide();
					$("#<?php echo $sWindowId?> #browsersDiagram span[id ^= 'pieLabel" + obj.seriesIndex + "']").show();
				});

				placeholderBrowsersDiagram.resize(function(){$("#<?php echo $sWindowId?> #browsersDiagram span[id ^= 'pieLabel']").hide();});
