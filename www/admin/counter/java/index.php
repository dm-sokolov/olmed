...
// Код формы
$iAdmin_Form_Id = 119;
$sAdminFormAction = '/admin/counter/java/index.php';

$sCounterPath = '/admin/counter/index.php';

$oAdmin_Form = Core_Entity::factory('Admin_Form', $iAdmin_Form_Id);

$sTitle = Core::_('Counter_Session.java');
...
!isset($oAdmin_Form_Controller->request['admin_form_filter_from_533']) && $oAdmin_Form_Controller->request['admin_form_filter_from_533'] = Core_Date::timestamp2date($iFromTimestamp);
!isset($oAdmin_Form_Controller->request['admin_form_filter_to_533']) &&	$oAdmin_Form_Controller->request['admin_form_filter_to_533'] = Core_Date::timestamp2date(time());
	...
	<div class="col-xs-12 col-lg-8">
		<div class="widget counter">
			<div class="widget-body">
				<div class="row">
					<div class="col-xs-12">
						<div id="javaDiagram" class="chart"></div>
					</div>
				</div>
			</div>
		</div>
	...
	<script type="text/javascript">
		$(function(){
			...
			foreach ($aObjects as $key => $oObject)
			{
				...
				diagramData.push(
					{
						label:'<?php echo $oObject->java ? Core::_('Counter_Session.yes') : Core::_('Counter_Session.not');?>',
			...
			?>
			var placeholderJavaDiagram = $("#<?php echo $sWindowId?> #javaDiagram");

			$.plot(placeholderJavaDiagram, diagramData, {
				series: {
					pie: {
						show: true,
						radius: 1,
			...
			placeholderJavaDiagram.bind("plothover", function (event, pos, obj) {
				if (!obj) {
					return;
				}

				$("#<?php echo $sWindowId?> #javaDiagram span[id ^= 'pieLabel']").hide();
				$("#<?php echo $sWindowId?> #javaDiagram span[id ^= 'pieLabel" + obj.seriesIndex + "']").show();
			});

			placeholderJavaDiagram.resize(function(){$("#<?php echo $sWindowId?> #javaDiagram span[id ^= 'pieLabel']").hide();});
