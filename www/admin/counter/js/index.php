...
// Код формы
$iAdmin_Form_Id = 120;
$sAdminFormAction = '/admin/counter/js/index.php';

$sCounterPath = '/admin/counter/index.php';

$oAdmin_Form = Core_Entity::factory('Admin_Form', $iAdmin_Form_Id);

$sTitle = Core::_('Counter_Session.js');
...
!isset($oAdmin_Form_Controller->request['admin_form_filter_from_537']) && $oAdmin_Form_Controller->request['admin_form_filter_from_537'] = Core_Date::timestamp2date($iFromTimestamp);
!isset($oAdmin_Form_Controller->request['admin_form_filter_to_537']) &&	$oAdmin_Form_Controller->request['admin_form_filter_to_537'] = Core_Date::timestamp2date(time());
	...
	<div class="col-xs-12 col-lg-8">
		<div class="widget counter">
			<div class="widget-body">
				<div class="row">
					<div class="col-xs-12">
						<div id="jsDiagram" class="chart"></div>
					</div>
				</div>
			</div>
		</div>
	...
	<script type="text/javascript">
		$(function(){
			...
			foreach ($aObjects as $key => $oObject)
			{
				...
				diagramData.push(
					{
						label:'<?php echo Core_Str::escapeJavascriptVariable(htmlspecialchars($oObject->js()))?>',
			...
			?>
			var placeholderJsDiagram = $("#<?php echo $sWindowId?> #jsDiagram");

			$.plot(placeholderJsDiagram, diagramData, {
				series: {
					pie: {
						show: true,
						radius: 1,
			...
			placeholderJsDiagram.bind("plothover", function (event, pos, obj) {
				if (!obj) {
					return;
				}

				$("#<?php echo $sWindowId?> #jsDiagram span[id ^= 'pieLabel']").hide();
				$("#<?php echo $sWindowId?> #jsDiagram span[id ^= 'pieLabel" + obj.seriesIndex + "']").show();
			});

			placeholderJsDiagram.resize(function(){$("#<?php echo $sWindowId?> #jsDiagram span[id ^= 'pieLabel']").hide();});
