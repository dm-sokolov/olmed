...
// Код формы
$iAdmin_Form_Id = 111;
$sAdminFormAction = '/admin/counter/os/index.php';

$sCounterPath = '/admin/counter/index.php';

$oAdmin_Form = Core_Entity::factory('Admin_Form', $iAdmin_Form_Id);

$sTitle = Core::_('Counter.os');
...
$oAdmin_Form_Dataset = new Admin_Form_Dataset_Entity(
	Core_Entity::factory('Counter_Os')
);

$aSetting = Core_Array::get(Core::$config->get('counter_setting'), 'setting', array());

(!isset($oAdmin_Form_Controller->request['admin_form_filter_from_452'])
	|| $oAdmin_Form_Controller->request['admin_form_filter_from_452'] == '')
		&& $oAdmin_Form_Controller->request['admin_form_filter_from_452'] = Core_Date::timestamp2date(time());

(!isset($oAdmin_Form_Controller->request['admin_form_filter_to_452'])
	|| $oAdmin_Form_Controller->request['admin_form_filter_to_452'] == '')
		&& $oAdmin_Form_Controller->request['admin_form_filter_to_452'] = Core_Date::timestamp2date(time());

$bSeveralDates = $oAdmin_Form_Controller->request['admin_form_filter_from_452'] != $oAdmin_Form_Controller->request['admin_form_filter_to_452'];
	...
	?><div class="col-xs-12 col-lg-8">
		<div class="widget counter">
			<div class="widget-body">
				<div class="row">
					<div class="col-xs-12">
						<div id="osDiagram" class="chart"></div>
					</div>
				</div>
			</div>
		</div>
		...
		$(function(){
			...
			$.getMultiContent(aScripts, '/modules/skin/bootstrap/js/charts/flot/').done(function() {
				...
				foreach ($aObjects as $key => $oObject)
				{
					...
					diagramData.push(
						{
							label: '<?php echo Core_Str::escapeJavascriptVariable(htmlspecialchars(Core_Str::cut($oObject->os, $aSetting['nameLen'])))?>',
				...
				?>
				var placeholderOsDiagram = $("#<?php echo $sWindowId?> #osDiagram");

				placeholderOsDiagram.unbind();

				$.plot(placeholderOsDiagram, diagramData, {
					series: {
						pie: {
							show: true,
							radius: 1,
				...
				placeholderOsDiagram.bind("plothover", function (event, pos, obj) {
					if (!obj) {
						return;
					}

					$("#<?php echo $sWindowId?> #osDiagram span[id ^= 'pieLabel']").hide();
					$("#<?php echo $sWindowId?> #osDiagram span[id ^= 'pieLabel" + obj.seriesIndex + "']").show();
				});
				placeholderOsDiagram.resize(function(){$("#<?php echo $sWindowId?> #browsersDiagram span[id ^= 'pieLabel']").hide();});

				$("#<?php echo $sWindowId?> span[id ^= 'pieLabel']").hide();
			});
		});
	</script>
