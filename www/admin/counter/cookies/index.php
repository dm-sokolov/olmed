...
// Код формы
$iAdmin_Form_Id = 118;
$sAdminFormAction = '/admin/counter/cookies/index.php';

$sCounterPath = '/admin/counter/index.php';

$oAdmin_Form = Core_Entity::factory('Admin_Form', $iAdmin_Form_Id);

$sTitle = Core::_('Counter_Session.cookies');
...
!isset($oAdmin_Form_Controller->request['admin_form_filter_from_532']) && $oAdmin_Form_Controller->request['admin_form_filter_from_532'] = Core_Date::timestamp2date($iFromTimestamp);
!isset($oAdmin_Form_Controller->request['admin_form_filter_to_532']) &&	$oAdmin_Form_Controller->request['admin_form_filter_to_532'] = Core_Date::timestamp2date(time());
	...
	?><div class="col-xs-12 col-lg-8">
		<div class="widget counter">
			<div class="widget-body">
				<div class="row">
					<div class="col-xs-12">
						<div id="cookiesDiagram" class="chart"></div>
					</div>
				</div>
			</div>
		</div>
	...
	<script type="text/javascript">
		$(function(){
			...
			foreach ($aObjects as $key => $oObject)
			{
				...
				diagramData.push(
					{
						label:'<?php echo $oObject->cookies ? Core::_('Counter_Session.yes') : Core::_('Counter_Session.not')?>',
			...
			?>
			var placeholderCookiesDiagram = $("#<?php echo $sWindowId?> #cookiesDiagram");

			$.plot(placeholderCookiesDiagram, diagramData, {
				series: {
					pie: {
						show: true,
						radius: 1,
			...
			placeholderCookiesDiagram.bind("plothover", function (event, pos, obj) {
				if (!obj) {
					return;
				}

				$("#<?php echo $sWindowId?> #cookiesDiagram span[id ^= 'pieLabel']").hide();
				$("#<?php echo $sWindowId?> #cookiesDiagram span[id ^= 'pieLabel" + obj.seriesIndex + "']").show();
			});

			placeholderCookiesDiagram.resize(function(){$("#<?php echo $sWindowId?> #cookiesDiagram span[id ^= 'pieLabel']").hide();});
