...
// Код формы
$iAdmin_Form_Id = 108;
$sAdminFormAction = '/admin/counter/searchsystem/index.php';

$sCounterPath = '/admin/counter/index.php';

$oAdmin_Form = Core_Entity::factory('Admin_Form', $iAdmin_Form_Id);

$sTitle = Core::_('Counter.searchsystem');
...
(!isset($oAdmin_Form_Controller->request['admin_form_filter_from_446'])
	|| $oAdmin_Form_Controller->request['admin_form_filter_from_446'] == '')
		&& $oAdmin_Form_Controller->request['admin_form_filter_from_446'] = Core_Date::timestamp2date(time());

(!isset($oAdmin_Form_Controller->request['admin_form_filter_to_446'])
	|| $oAdmin_Form_Controller->request['admin_form_filter_to_446'] == '')
		&& $oAdmin_Form_Controller->request['admin_form_filter_to_446'] = Core_Date::timestamp2date(time());

$bSeveralDates = $oAdmin_Form_Controller->request['admin_form_filter_from_446'] != $oAdmin_Form_Controller->request['admin_form_filter_to_446'];
...
$bSeveralDates && $oAdmin_Form_Dataset->addCondition(
	array('groupBy' => array('searchsystem'))
);
		...
		$.getMultiContent(aScripts, '/modules/skin/bootstrap/js/charts/flot/').done(function() {
			...
			foreach ($aObjects as $key => $oObject)
			{
				...
				searchSystemDiagramData.push(
					{
						label: '<?php echo Core_Str::escapeJavascriptVariable(htmlspecialchars($oObject->searchsystem))?>',
