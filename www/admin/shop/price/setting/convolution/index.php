...
$sAdminFormAction = '/admin/shop/price/setting/convolution/index.php';
...
$oAdmin_Form_Controller
	...
	->title(Core::_('Shop_Price_Setting_Convolution.title'));
...
$oAdmin_View
	->module(Core_Module::factory($sModule))
	->pageTitle(Core::_('Shop_Price_Setting_Convolution.title'));
...
if ($bStart)
{
	...
	$Shop_Price_Setting_Convolution_Controller = new Shop_Price_Setting_Convolution_Controller();

	if (isset($_SESSION['Shop_Price_Setting_Convolution_Controller']))
	{
		$Shop_Price_Setting_Convolution_Controller = $_SESSION['Shop_Price_Setting_Convolution_Controller'];
		unset($_SESSION['Shop_Price_Setting_Convolution_Controller']);
	}
	else
	{
		$Shop_Price_Setting_Convolution_Controller = new Shop_Price_Setting_Convolution_Controller();
	...
	$bCompleted = $Shop_Price_Setting_Convolution_Controller
		->execute();

	$sAdditionalParams = "shop_id={$iShopId}&shop_group_id={$iShopGroupId}&date={$sDate}&limit={$iLimit}";
	...
	if ($mode == 'import')
	{
		...
		{
			case 'finish':
				Core_Message::show(Core::_('Shop_Price_Setting_Convolution.create_end_post_start'), 'warning');
				$mode = 'post';
			break;
			case 'continue':
				Core_Message::show(Core::_('Shop_Price_Setting_Convolution.create_document', $Shop_Price_Setting_Convolution_Controller->position), 'warning');
			break;
		}

		$_SESSION['Shop_Price_Setting_Convolution_Controller'] = $Shop_Price_Setting_Convolution_Controller;
	...
	{
		// Post step-by-step
		if ($Shop_Price_Setting_Convolution_Controller->postNext())
		{
			$_SESSION['Shop_Price_Setting_Convolution_Controller'] = $Shop_Price_Setting_Convolution_Controller;

			$sRedirectAction = $oAdmin_Form_Controller->getAdminActionLoadAjax($oAdmin_Form_Controller->getPath(), 'start', NULL, 0, 0, $sAdditionalParams . '&mode=' . $mode);

			Core_Message::show(Core::_('Shop_Price_Setting_Convolution.post'), 'warning');
		}
		else
		{
			$sRedirectAction = "";

			Core_Message::show(Core::_('Shop_Price_Setting_Convolution.create_end'));
		}
	}
	else
	{
...
if (!$bStart || $bCompleted)
{
	...
	$oMainRow1->add(
		Admin_Form_Entity::factory('Date')
			...
			->caption(Core::_('Shop_Price_Setting_Convolution.date'))
			->name('date')
			->value(Core_Date::date2sql(date('Y/m/d', strtotime('-1 days'))))
	)->add(
		Admin_Form_Entity::factory('Input')
			->divAttr(array('class' => 'form-group col-xs-12 col-sm-2'))
			->caption(Core::_('Shop_Price_Setting_Convolution.limit'))
			->name('limit')
			->value(5000)
	)->add(
		Admin_Form_Entity::factory('Input')
		...
		->add(
			Admin_Form_Entity::factory('Button')
				->name('start')
				->type('submit')
				->value(Core::_('Shop_Price_Setting_Convolution.start'))
	...
	$oAdmin_Form_Entity_Breadcrumbs->add(
		...
		Admin_Form_Entity::factory('Breadcrumb')
			->name(Core::_('Shop_Price_Setting.title'))
			->href($oAdmin_Form_Controller->getAdminLoadHref('/admin/shop/price/setting/index.php', NULL, NULL, $sAdditionalParams = "shop_id={$oShop->id}&shop_group_id={$shop_group_id}"))
			->onclick($oAdmin_Form_Controller->getAdminLoadAjax('/admin/shop/price/setting/index.php', NULL, NULL, $sAdditionalParams))
	);

	// Элементы строки навигации
	$oAdmin_Form_Entity_Breadcrumbs->add(
		Admin_Form_Entity::factory('Breadcrumb')
			->name(Core::_('Shop_Price_Setting_Convolution.title'))
			->href(
