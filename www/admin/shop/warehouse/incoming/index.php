...
// Код формы
$iAdmin_Form_Id = 253;
$sAdminFormAction = '/admin/shop/warehouse/incoming/index.php';
...
$sFormTitle = Core::_('Shop_Warehouse_Incoming.title');
...
{
	$Shop_Warehouse_Incoming_Controller_Edit = Admin_Form_Action_Controller::factory(
		'Shop_Warehouse_Incoming_Controller_Edit', $oAdmin_Form_Action
	);

	$Shop_Warehouse_Incoming_Controller_Edit->addEntity($oAdmin_Form_Entity_Breadcrumbs);

	// Добавляем типовой контроллер редактирования контроллеру формы
	$oAdmin_Form_Controller->addAction($Shop_Warehouse_Incoming_Controller_Edit);
}
...
{
	$Shop_Warehouse_Incoming_Item_Controller_Delete = Admin_Form_Action_Controller::factory(
		'Shop_Warehouse_Incoming_Item_Controller_Delete', $oAdminFormActionDeleteShopItem
	);

	$oAdmin_Form_Controller->addAction($Shop_Warehouse_Incoming_Item_Controller_Delete);
}
...
{
	$Shop_Warehouse_Incoming_Controller_Print = Admin_Form_Action_Controller::factory(
		'Shop_Warehouse_Incoming_Controller_Print', $oAdmin_Form_Action
	);

	$Shop_Warehouse_Incoming_Controller_Print
		->title(Core::_('Shop_Warehouse_Incoming.title'))
		->printlayout($printlayout_id);

	// Добавляем типовой контроллер редактирования контроллеру формы
	$oAdmin_Form_Controller->addAction($Shop_Warehouse_Incoming_Controller_Print);
}
...
$oAdmin_Form_Dataset = new Admin_Form_Dataset_Entity(
	Core_Entity::factory('Shop_Warehouse_Incoming')
);
...
$oAdmin_Form_Dataset->addCondition(
	array('select' => array('shop_warehouse_incomings.*'))
)
->addCondition(
	array(
		'leftJoin' => array('shop_warehouses', 'shop_warehouses.id', '=', 'shop_warehouse_incomings.shop_warehouse_id')
	)
)
->addCondition(
	array(
