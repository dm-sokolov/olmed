...
$sAdminFormAction = '/admin/siteuser/status/index.php';
...
$pageTitle = Core::_('Siteuser_Status.siteuser_statuses_title');
...
$oAdminFormEntityBreadcrumbs
	->add(
		Admin_Form_Entity::factory('Breadcrumb')
			...
			->href(
				$oAdmin_Form_Controller->getAdminLoadHref($sSiteuserPath)
			)
			->onclick(
				$oAdmin_Form_Controller->getAdminLoadAjax($sSiteuserPath)
			)
	)
	->add(
		Admin_Form_Entity::factory('Breadcrumb')
...
{
	$oSiteuser_Status_Controller_Edit = Admin_Form_Action_Controller::factory(
		'Siteuser_Status_Controller_Edit', $oAdminFormAction
	);

	// Добавляем контроллер редактирования контроллеру формы
	$oAdmin_Form_Controller->addAction($oSiteuser_Status_Controller_Edit);

	// Крошки при редактировании
	$oSiteuser_Status_Controller_Edit->addEntity($oAdminFormEntityBreadcrumbs);
}
...
$oAdmin_Form_Dataset = new Admin_Form_Dataset_Entity(
	Core_Entity::factory('Siteuser_Status')
);
