...
class Core_Html_Entity_Select extends Core_Html_Entity
{
    ...
    public function execute()
    {
        ...
        ?><div <?php echo implode(' ', $aAttr) ?>><?php
    ...
    {
        ?><a class="select__link" value="<?php echo htmlspecialchars($value)?>" data-link="<?php echo htmlspecialchars($key)?>" href="http://<?php echo htmlspecialchars($key)?>"<?php
        foreach ($aAttr as $attrKey => $attrValue)
        {
            echo ' ', $attrKey, '=', '"', htmlspecialchars($attrValue, ENT_COMPAT, 'UTF-8'), '"';
        }
        ?>><?php echo htmlspecialchars($value, ENT_COMPAT, 'UTF-8')?></a><?php
    }
}