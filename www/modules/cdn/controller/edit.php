...
class Cdn_Controller_Edit extends Admin_Form_Action_Controller_Type_Edit
{
	...
	public function setObject($object)
	{
		...
		$this->title(
			$this->_object->id
				? Core::_('Cdn.edit_title', $this->_object->name)
				: Core::_('Cdn.add_title')
		);

		$this->addSkipColumn('balance');
		...
		$oMainTab->delete($this->getField('balance'));
		...
		{
			try {
				$Cdn_Controller = Cdn_Controller::instance($object->driver);
				$Cdn_Controller->setCdn($object);
