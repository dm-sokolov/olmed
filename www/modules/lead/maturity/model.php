 ...
 */
class Lead_Maturity_Model extends Core_Entity
{
	/**
	...
	public function nameBackend()
	{
		return '<i class="fa fa-circle" style="margin-right: 5px; color: ' . ($this->color ? htmlspecialchars($this->color) : '#aebec4') . '"></i> '
			. '<span class="editable" id="apply_check_0_' . $this->id . '_fv_1533">' . htmlspecialchars($this->name) . '</span>';
	}
	...
	public function delete($primaryKey = NULL)
	{
		...
		Core_QueryBuilder::update('leads')
			->set('lead_maturity_id', 0)
			->where('lead_maturity_id', '=', $this->id)
			->execute();
