	...
	public function execute($operation = NULL)
	{
		...
		if ($operation == 'finish')
		{
				...
				$oCore_Html_Entity_Form
					->add(
						Admin_Form_Entity::factory('Div')
							->class('col-xs-12 col-sm-6 lead-exist-client hidden')
							->add($oSiteuserSelect)
					);
			}
			...
			Core::factory('Core_Html_Entity_Script')
				->value("$(function() {
					$('#{$newWindowId}').HostCMSWindow({ autoOpen: true, destroyOnClose: false, title: '" . $this->title . "', AppendTo: '#{$windowId}', width: 550, height: 100, addContentPadding: true, modal: false, Maximize: false, Minimize: false }); });")
				->execute();
