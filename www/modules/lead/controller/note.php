...
class Lead_Controller_Note extends Admin_Form_Controller_View
{
	...
	protected function _showContent()
	{
		...
		$additionalParams = 'lead_id={lead_id}';
		...
		$lead_id = intval(Core_Array::getGet('lead_id', 0));
		$oLead = Core_Entity::factory('Lead', $lead_id);
		...
		<div class="deal-note-board">
			<div class="row">
				<div class="col-xs-12 margin-bottom-20">
					<form action="/admin/lead/note/index.php" method="POST" class="padding-bottom-10">
						<div class="input-group">
							<textarea rows="2" name="text_note" type="text" class="form-control" placeholder="<?php echo Core::_('Lead_Note.note_placeholder')?>"></textarea>
										...
										if ($oAdmin_Form->show_operations)
										{
											...
											foreach ($aAllowed_Admin_Form_Actions as $oAdmin_Form_Action)
											{
												...
												$onclick = $oAdmin_Form_Action->name == 'edit'
													? "$.modalLoad({path: '{$oAdmin_Form_Controller->getPath()}', action: 'edit', operation: 'modal', additionalParams: 'hostcms[checked][0][{$oEntity->id}]=1&lead_id={$oLead->id}', windowId: '{$windowId}'}); return false"
