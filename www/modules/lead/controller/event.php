 ...
 */
class Lead_Controller_Event extends Admin_Form_Controller_View
{
	/**
						...
						foreach ($aEntities as $oEvent)
						{
							...
							<tr class="deal-event-row">
								...
								<td class="hidden-xxs" style="text-align:left; width: 25%">
									...
									<div class="pull-right deal-event-actions">
									<?php
									...
									if ($oAdmin_Form->show_operations)
									{
										...
										foreach ($aAllowed_Admin_Form_Actions as $oAdmin_Form_Action)
										{
											...
											$onclick = $oAdmin_Form_Action->name == 'edit'
												? "$.modalLoad({path: '{$oAdmin_Form_Controller->getPath()}', action: 'edit', operation: 'modal', additionalParams: 'hostcms[checked][0][{$oEvent->id}]=1&lead_id={$oLead->id}', windowId: '{$windowId}'}); return false"
