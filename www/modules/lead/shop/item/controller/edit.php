...
class Lead_Shop_Item_Controller_Edit extends Admin_Form_Action_Controller_Type_Edit
{
	...
	public function setObject($object)
	{
		...
		$aCurrencySelectOptions = array();

		foreach ($aShop_Currencies as $aShop_Currency)
		{
			$aCurrencySelectOptions[$aShop_Currency->id] = $aShop_Currency->name;
		}
		...
		$oShopCurrencySelect = Admin_Form_Entity::factory('Select')
			...
			->options($aCurrencySelectOptions)
			->id('shopCurrencyId')
		...
		$title = $this->_object->id
			...
			: Core::_('Lead_Shop_Item.lead_shop_items_add_form_title');
	...
	public function execute($operation = NULL)
	{
		...
		$sJsRefresh = '<script>
			// Refresh lead shop items list
			if ($("#lead-shop-items").length)
			{
				$.adminLoad({ path: \'/admin/lead/shop/item/index.php\', additionalParams: \'lead_id=' . $iLeadId . '\', windowId: \'lead-shop-items\' });
			}
		</script>';
