 ...
 */
class Cloud_Yandex_Controller extends Cloud_Controller
{
	 ...
	 */
	public function download($sFileName, $sTargetDir)
	{
		if ($this->_token)
		{
			...
			if (!count($aFileData))
			{
				...
				$aFileData = json_decode($Core_Http->getBody(), TRUE);
	...
	public function upload($sSourceFileName, $sTargetDirectory)
	{
		...
		if ($this->_token)
		{
			...
			$sTargetPath = urlencode($this->dir . $sSourceFileName);
			...
			$oFileData = json_decode($Core_Http->getBody());
			...
			$sFilePath = $sTargetDirectory . $sSourceFileName;
			...
			if (file_exists($sFilePath))
			{
				...
				$aConfig['options'][CURLOPT_INFILESIZE] = Core_File::filesize($sFilePath);
