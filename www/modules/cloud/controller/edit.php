...
class Cloud_Controller_Edit extends Admin_Form_Action_Controller_Type_Edit
{
	...
	public function setObject($object)
	{
		...
		$title = is_null($this->_object->id)
			...
			: Core::_('Cloud.edit_title', $this->_object->name);
		...
		$oCodeLink
			...
			->a
				...
				->onclick("var id = $('#{$windowId} input[name=\'id\']');
					...
					{
						$.ajaxRequest({path: '/admin/cloud/index.php', datasetId: 0, objectId: id.val(), context: 'document_id', callBack: function(data, status, jqXHR){
							$.loadingScreen('hide');
