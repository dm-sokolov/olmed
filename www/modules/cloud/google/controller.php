 ...
 */
class Cloud_Google_Controller extends Cloud_Controller
{
	/**
	 * Redirect uri
	 */
	...
	protected function _getDirInfo($sGroupId)
	{
		...
		if ($AccessToken)
		{
			...
			$oGoogle_Service_Drive_File = json_decode($Core_Http->getBody());
	...
	protected function _getDirId($sParentId, $sDirectoryName)
	{
		...
		if ($AccessToken)
		{
			try
			{
				...
				$oGoogle_Service_Drive_Folders = json_decode($Core_Http->getBody())->files[0];
				...
				if ($sParentId == 'root')
				{
					...
					$oRootData = json_decode($Core_Http->getBody());
				}

				return count($oGoogle_Service_Drive_Folders)
	...
	public function listDir()
	{
		...
		if ($AccessToken)
		{
			...
			if (!isset(self::$_cache[$sDirectory]))
			{
				...
				$oGoogle_Service_Drive_FilesAndFolders = json_decode($Core_Http->getBody());
	 ...
	 */
	public function download($fileId, $sTargetDir)
	{
		$AccessToken = $this->getToken();

		if ($AccessToken)
		{
			$sFileName = base64_decode($fileId);
	...
	public function upload($sSourceFileName, $sTargetDirectory)
	{
		...
		if ($AccessToken)
		{
			$sSourcePath = $sTargetDirectory . $sSourceFileName;

			$sFileMime = Core_Mime::getFileMime($sSourcePath);
			...
			//$sContent = "{\"name\":\"{$sSourceFileName}\", \"parents\": [\"{$sDirectory}\"]}";
			...
			$originalFilename = $sSourceFileName;
			...
			if (is_null(Core_Array::get($aConformityFileNameToFileID, $originalFilename)))
			{
				...
				$oGoogle_Service_Drive_Files = json_decode($Core_Http->getBody());
