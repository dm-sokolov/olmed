...
class Siteuser_Oauth_Facebook_Controller extends Siteuser_Oauth_Controller
{
	...
	public function execute()
	{
		...
		{
			echo "<script>location.href = 'https://www.facebook.com/dialog/oauth?scope=email&client_id={$this->APPLICATION_ID}&redirect_uri={$this->CALLBACK_URL}';</script>";
			return NULL;
		}
		else
		{
			$sAnswer = @file_get_contents("https://graph.facebook.com/oauth/access_token?client_id={$this->APPLICATION_ID}&redirect_uri={$this->CALLBACK_URL}&client_secret={$this->APPLICATION_SECRET}&code={$this->code}", FALSE, stream_context_create(array(
				'http' => array(
						'method' => 'GET'
					)
				)
			...
			if (strlen($sAnswer))
			{
				...
				$sAnswer = @file_get_contents("https://graph.facebook.com/me?access_token={$aReturnData['access_token']}&fields=email,first_name,last_name,picture", FALSE, stream_context_create(array(
					'http' => array(
							'method' => 'GET'
						)
					)
