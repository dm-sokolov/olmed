...
class Siteuser_Email_Model extends Core_Entity
{
	...
	{
		 return 'upload/private/email/' . Core_File::getNestingDirPath($this->id, 3) . '/' . $this->id . '/';
	}
	...
	public function subjectBackend($oAdmin_Form_Field, $oAdmin_Form_Controller)
	{
		...
		<script>
		$(function() {
			$('a#siteuser_email<?php echo $this->id?>').on('click', function (){
				$.ajax({
					url: '/admin/siteuser/email/index.php',
					type: "POST",
	...
	public function importantBackend($oAdmin_Form_Field, $oAdmin_Form_Controller)
	{
		return $this->important
			? '<i class="fa fa-exclamation-circle darkorange" title="' . Core::_('Siteuser_Email.important') . '"></i>'
			: '';
	}
