...
class Siteuser_Model extends Core_Entity
{
	...
	public function indexing()
	{
		...
		$oSearch_Page->text = htmlspecialchars($this->login) . ' ' . htmlspecialchars($this->email);

		// People
		$aSiteuser_People = $this->Siteuser_People->findAll(FALSE);
		foreach ($aSiteuser_People as $oSiteuser_Person)
		{
			$oSearch_Page->text .= ' ' . htmlspecialchars($oSiteuser_Person->name) . ' ' .
				htmlspecialchars($oSiteuser_Person->surname) . ' ' .
				htmlspecialchars($oSiteuser_Person->patronymic) . ' ' .
				htmlspecialchars($oSiteuser_Person->post) . ' ' .
			...
			foreach ($aDirectory_Phones as $oDirectory_Phone)
			{
				$oDirectory_Phone->public
					&& $oSearch_Page->text .= ' ' . htmlspecialchars($oDirectory_Phone->value);
			}
			...
			foreach ($aDirectory_Websites as $oDirectory_Website)
			{
				$oDirectory_Website->public
					&& $oSearch_Page->text .= ' ' . htmlspecialchars($oDirectory_Website->value);
			}
			...
			foreach ($aDirectory_Emails as $oDirectory_Email)
			{
				$oDirectory_Email->public
					&& $oSearch_Page->text .= ' ' . htmlspecialchars($oDirectory_Email->value);
			}
			...
			foreach ($aDirectory_Socials as $oDirectory_Social)
			{
				$oDirectory_Social->public
					&& $oSearch_Page->text .= ' ' . htmlspecialchars($oDirectory_Social->value);
			}
			...
			foreach ($aDirectory_Messengers as $oDirectory_Messenger)
			{
				$oDirectory_Messenger->public
					&& $oSearch_Page->text .= ' ' . htmlspecialchars($oDirectory_Messenger->value);
			}
		}
		...
		{
			$oSearch_Page->text .= ' ' . htmlspecialchars($oSiteuser_Company->name);
			...
			foreach ($aDirectory_Addresses as $oDirectory_Address)
			{
				$oDirectory_Address->public
					&& $oSearch_Page->text .= ' ' . htmlspecialchars($oDirectory_Address->postcode) . ' ' .
						htmlspecialchars($oDirectory_Address->country) . ' ' .
						htmlspecialchars($oDirectory_Address->city) . ' ' .
						htmlspecialchars($oDirectory_Address->value);
			}
	...
	protected function _prepareData()
	{
		$this->clearXmlTags()
			...
			->addXmlTag('datetime', strftime($this->Site->date_time_format, Core_Date::sql2timestamp($this->datetime)))
			->addXmlTag('date', strftime($this->Site->date_format, Core_Date::sql2timestamp($this->datetime)))
			->addXmlTag('path', urlencode($this->login))
	...
	public function loginBackend()
	{
		...
		if ($this->crm_source_id)
		{
			...
			$sResult .= '&nbsp;<i title="' . htmlspecialchars($oCrm_Source->name) . '" class="fa ' . htmlspecialchars($oCrm_Source->icon) . ' fa-small" style="color: ' . htmlspecialchars($oCrm_Source->color) . '"></i>';
		}
		...
		if (count($aSiteuser_Groups))
		{
			...
			foreach ($aSiteuser_Groups as $oSiteuser_Group)
			{
				$sListSiteuserGroups .= '<i class="fa fa-folder-open-o" style="margin-right: 5px"></i><a onclick="'
					. "$.adminLoad({path: '/admin/siteuser/group/list/index.php', additionalParams: 'siteuser_group_id=" . $oSiteuser_Group->id . "', windowId: 'id_content'}); return false"
					. '">' . htmlspecialchars($oSiteuser_Group->name) . "</a><br/>";
			}

			$sResult .= '<a id="siteuser_' . $this->id . '" type="button" class="siteuser_group_list_link" style="margin-right: 5px;" data-toggle="popover" data-placement="right" data-content="' . htmlspecialchars($sListSiteuserGroups) . '" data-title="' . Core::_('Siteuser_Group.title') . '" data-titleclass="bordered-darkorange" data-container="#siteuser_' . $this->id . '"><i class="fa fa-folder-o gray"></i></a>';
		}
	...
	public function merge(Siteuser_Model $oObject)
	{
		...
		foreach ($aSiteuser_People as $oSiteuser_Person)
		{
			$this->add(clone $oSiteuser_Person);
		}
