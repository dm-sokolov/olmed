 ...
 */
class Cache_XCache extends Core_Cache
{
	/**
	...
	public function __construct($config)
	{
		if (!$this->available())
		{
			throw new Core_Exception('XCache extension does not exist', array(), 0, FALSE);
		}
		...
		{
			throw new Core_Exception('XCache caches configuration section does not exist', array(), 0, FALSE);
		}
	...
	public function set($key, $value, $cacheName = 'default', array $tags = array())
	{
		...
		if ($this->_config['caches'][$cacheName]['active'])
		{
			...
			if (strlen($valueToWrite) <= $this->_config['caches'][$cacheName]['size'])
			{
				...
				xcache_set($actualKey, $valueToWrite, $expire);
	...
	public function deleteAll($cacheName = 'default')
	{
		...
		// Очистка всего кэша системы или конкретного кэша по имени
		if (function_exists('xcache_unset_by_prefix'))
		{
			xcache_unset_by_prefix($actualKey);
		}
