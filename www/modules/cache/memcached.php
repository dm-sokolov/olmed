 ...
 */
class Cache_Memcached extends Core_Cache
{
	/**
	...
	public function __construct($config)
	{
		if (!$this->available())
		{
			throw new Core_Exception('Memcached extension does not exist', array(), 0, FALSE);
		}
		...
		{
			throw new Core_Exception('Memcached configuration section does not exist', array(), 0, FALSE);
		}
		...
		if (!is_null($config['username']) || !is_null($config['password']))
		{
			...
			$this->_memcached->setSaslAuthData($config['username'], $config['password']);
		}

		if (!$this->_memcached->addServer($config['server'], $config['port']))
		{
			Core_Log::instance()->clear()
				->status(Core_Log::$ERROR)
				->write('Memcached: can\'t add server!');
		}
	...
	public function check($key, $cacheName = 'default')
	{
		if ($this->_issetCacheConfig($cacheName) && $this->_config['caches'][$cacheName]['active'])
		{
			...
			$return = $this->_memcached->get($actualKey) !== FALSE;
		}
		else
		{
			$return = NULL;
	...
	public function get($key, $cacheName = 'default', $defaultValue = NULL)
	{
		...
		if ($this->_issetCacheConfig($cacheName) && $this->_config['caches'][$cacheName]['active'])
		{
			...
			$return = $this->_memcached->get($actualKey);
	...
	public function set($key, $value, $cacheName = 'default', array $tags = array())
	{
		...
		if ($this->_config['caches'][$cacheName]['active'])
		{
			...
			if (strlen($valueToWrite) <= $this->_config['caches'][$cacheName]['size'])
			{
				...
				$this->_memcached->set($actualKey, $valueToWrite, $expire);
