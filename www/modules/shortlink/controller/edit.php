...
class Shortlink_Controller_Edit extends Admin_Form_Action_Controller_Type_Edit
{
		...
		switch ($modelName)
		{
			case 'shortlink':
				...
				$oMainTab
					...
					->move($this->getField('active')->divAttr(array('class' => 'form-group col-xs-12 col-sm-6 col-md-3 margin-top-21')), $oMainRow3);
			...
			case 'shortlink_dir':
				...
				$title = $this->_object->id
					? Core::_('Shortlink_Dir.edit_form_title', htmlspecialchars($this->_object->name))
	...
	{
		$this->_formValues['source'] = trim(Core_Array::get($this->_formValues, 'source'));
		$this->_formValues['shortlink'] = trim(Core_Array::get($this->_formValues, 'shortlink'));
